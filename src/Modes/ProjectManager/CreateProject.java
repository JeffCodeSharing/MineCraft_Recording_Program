...
public class CreateProject extends Application implements AbstractWindow {
    ...
    public void draw_controls(Group group) {
        ...
        project_name.textProperty().addListener((observableValue, aBoolean, t1) -> update_labels(warning, create_path, project_path, project_name));
        project_path.textProperty().addListener((observableValue, aBoolean, t1) -> update_labels(warning, create_path, project_path, project_name));

        Button search_dir = WinTool.createButton(550, 135, 30, 30, 12, "...");
        search_dir.setOnAction(actionEvent -> choose_directory(project_path));
        Button create_button = WinTool.createButton(380, 480, 80, 40, 15, "创建");
        create_button.setOnAction(actionEvent -> {
            if (create_project(create_path.getText()) && other_type_write_in(seed_in, password_in)) {
        ...
        update_labels(warning, create_path, project_path, project_name);
     ...
     */
    private void choose_directory(TextField textField) {
        try {
     ...
     */
    private boolean create_project(String create_path) {
        path = create_path;
        ...
        try {
            ...
            File behavior_dir = new File(path + File.separator + "behavior");
            File todo_finish_dir = new File(path + File.separator + "behavior" + File.separator + "finish");
            File todo_doing_dir = new File(path + File.separator + "behavior" + File.separator + "doing");
            File now_doing_file = new File(path + File.separator + "behavior" + File.separator + "now_doing");
            if (!behavior_dir.mkdirs() || !todo_finish_dir.mkdirs() || !todo_doing_dir.mkdirs() || !now_doing_file.createNewFile()) {
                return false;
            }

            File backup_dir = new File(path + File.separator + "backup");
     ...
     */
    private void update_labels(Label warning, Label create_path, TextField project_path, TextField project_name) {
        File file = new File(project_path.getText() + File.separator + project_name.getText());
     ...
     */
    private boolean other_type_write_in(TextField... textFields) {
        try {
            // 初始化所有FileWriter
