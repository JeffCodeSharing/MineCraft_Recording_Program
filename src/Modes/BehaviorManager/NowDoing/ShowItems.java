...
public class ShowItems extends Application implements AbstractWindow {
    ...
    public ShowItems(String path) {
        ...
        List<String> temp = IOTool.read_file_as_arrayList(path);
        if (temp == null) {
    ...
    public void draw_controls(Group group) {
        ...
        save.setOnAction(actionEvent -> save_values(true));     // 弹出提示框，主动保存
        cancel.setOnAction(actionEvent -> {
            save_values(false);     // 不弹出提示框，被动保存
            global_stage.close();
        });
     ...
     * 保存任务列表到文件。
     *
     * @param type true --> 允许弹出提示框; false --> 不允许弹出提示框
     */
    private void save_values(boolean type) {
        // 加密处理
        file_data.replaceAll(EDTool::encrypt);

        boolean save_type = IOTool.override_file(path, file_data);
        if (type) {
            if (save_type) {
     ...
     * @param index 用户点击的索引
     */
    private void finish_item(int index) {
        if (index != -1) {
