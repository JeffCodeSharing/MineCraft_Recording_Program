...
public class DataController {
    ...
    public void add(int index, String s) {
        // 确定用户创建的信息的父类在第几级
        int flag = get_tab_num(values.get(index)[0]);

        // 定位创建信息的创建位置
        for (index++; index < values.size(); index++) {    // 从index的下一个开始尝试
            int compare = get_tab_num(values.get(index)[0]);
    ...
    public void remove(int index) {
        ...
        if (flag != 0) { // 如果不是一级父类的情况下
            ...
            for (int backup = index + 1; backup < values.size(); backup++) {
                ...
                int compare = get_tab_num(value_data[0]);
     ...
     * @return 开头的制表符个数
     */
    private int get_tab_num(String data) {
        // 统计字符串开头的制表符个数
        int count = 0;
