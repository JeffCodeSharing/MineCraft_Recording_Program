...
public class HelpDocument extends Application implements AbstractWindow {
    ...
    private void show_details(boolean is_menu) {
        // 绘制Label
        ...
        if (!is_menu) {
            ...
            go_back.setOnAction(actionEvent -> {
                ...
                show_details(search_path.equals(System.getProperty("user.dir")
        ...
        for (String s:list) {
            ...
            if (!s.endsWith(".png") && !s.endsWith(".jpg") && !s.endsWith(".jpeg")) {
                ...
                label.setOnMousePressed(mouseEvent -> open_next_layer(s));
                pane_box.getChildren().addAll(label);
            }
        }
    }

    private void show_underlying_data() {
        try {
            String[] file_value = IOTool.read_file(search_path);
    ...
    private void open_next_layer(String layer_name) {
