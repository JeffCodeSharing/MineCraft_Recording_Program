...
public class CheckPassword extends Application implements AbstractWindow {
    ...
    public CheckPassword(String path) {
        if (!path.equals("none")) {
            try {
                ...
                String encrypt_password = IOTool.read_file(path + File.separator + "check_item")[2];     // 第三行的数据为密码
    ...
    public void draw_controls(Group group) {
        ...
        view.setOnMousePressed(mouseEvent -> image_click(view, password_plaintext, password));

        Button confirm = WinTool.createButton(100, 150, 80, 40, 16, "确定");
        Button cancel = WinTool.createButton(190, 150, 80, 40, 16, "取消");

        confirm.setOnAction(actionEvent -> after_confirm(password.getText()));
    ...
    private void after_confirm(String input) {
